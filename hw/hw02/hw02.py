...
def filtered_accumulate(combiner, base, pred, n, term):
    ...
    True
    """
    # if pred(term(n)) == False:
    #     return accumulate(combiner, base, 0, term)
    # else:
    return accumulate(lambda n, m: combiner(n, m) if pred(m) else n, base, n, term)
...
def pingpong(n):
    ...
    True
    """
    # when its time to switch
    def helper_switch(dr, k, m):
        if n == m:
            return k
        if m % 7 == 0 or has_seven(m):
            return helper_switch(dr - 2, k + 1, m + 1)
            if dr == 1:
                return helper_switch(dr - 2, k - 1, m + 1)
            elif dr == -1:
                return helper_switch(dr + 2, k + 1, m + 1)
            ...
            if dr == 1:
                return helper_switch(dr, k + 1, m + 1)
            elif dr == -1:
                return helper_switch(dr, k - 1, m + 1)
    return helper_switch(1, 1, 1)
...
def count_change(amount):
    ...
    # to find the next biggest # thats power of 2
    k = 1
    while k < amount:
        k *= 2
    maxpiece = k
...
def move_stack(n, start, end):
    ...
    # move 3 of them to the middle and then 
    # recursive call to move one on the left to the rightmost
    # for else case you need move 3 cases
    # you have some set of stack you break them down into n-1 and 1 
    ...
    # base case instructions if your only gonna move one from some rod to the other rod
    # how do we know how many discs we have left on one rod? => (sum of all the discs = 6) - (start + end)
    else:
        offset = 6 - (start + end)
