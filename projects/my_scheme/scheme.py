...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("number of inputted argument values does not match")
        else:
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN Question 12
        "*** REPLACE THIS LINE ***"
        # "self.env" b/c you need to access to its parent frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        paired_body = Pair(formals, body)
        env.define(name,do_lambda_form(paired_body, env))
        return name
        #LambdaProcedure(target.first, expressions.second, env) ?Could this be breaking abstraction barrier? by any chance?
        # END Question 9A
    else:
...
def do_if_form(expressions, env):
    ...
    # BEGIN Question 13
    "*** REPLACE THIS LINE ***"
    if scheme_false(expressions.first) or scheme_false(scheme_eval(expressions.first, env)):
...
def check_formals(formals):
    ...
    def check_helper(s):
        if s is nil:
            return True
        elif s.first in bank or not(scheme_symbolp(s.first)):
            return False
        else:
            bank.append(s.first)
            return check_helper(s.second)
    if not(scheme_listp(formals)):
        raise SchemeError("List must be well-formed")
    if not(check_helper(formals)):
        raise SchemeError("List must be well-formed and not be repeated")
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Evaluate):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            if not scheme_procedurep(procedure):
                raise SchemeError("cannot call: {0}".format(str(procedure)))
            result = procedure.eval_call(rest, env, True)
    return result
