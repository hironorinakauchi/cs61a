...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        if opponent_score < 10: # if oponent_score is just one digit # (1~9)
            return max([opponent_score, opponent_score // 10]) + 1
        else: # 10 for example
            return max([opponent_score%1000%100//10, opponent_score%10]) + 1
    # for i in range(0, len(opponent_score_str)): 
        #     print(i)
            # list1 = opponent_score_split.append(opponent_score[i])
            # print(list1)
    # END Question 2
