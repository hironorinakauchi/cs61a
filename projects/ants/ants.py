...
#Q1. What is the significance of an insect's armor attribute? What happens when armor reaches 0?
#A. HP = life points, when armor reaces 0, the insect will be removed.
#Q2. What are all of the attributes of the Ant class?
#A. is_ant, implemented, food_cost, armor, place, Insects(??) (could be wrond)
#Q3. Is the armor attribute of the Ant class an instance attribute or class attribute?
#A. instance attribute
#Q4. Is the damage attribute of the Ant class an instance attribute or class attribute?
#  class attribute
#Q5. Which class do both Ant and Bee inherit from?
# Insect
#Q6. What attribute(s) do Ant and Bee inherit from their base class?
# armor, place, damage, is_ant, 
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 6A
        if self.place.ant == None or self.place.ant.blocks_path == False: # if no ant in bee's place or if there's an ant buts its blocks_path => false
            return False
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 3A
        Place.add_insect(self, insect) #places an ant to the actual place, but returns None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        # BEGIN Problem 4A
        duplicated_list_of_bees = list(self.place.bees) # any bees in the same place as fireants?
        if self.armor <= amount: 
            for bee in duplicated_list_of_bees:
                bee.reduce_armor(self.damage)
        Ant.reduce_armor(self, amount) # kill the ants afterward
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """WallAnt does nothing but stands still. A WallAnt has a 4 armor value, which can be useful"""
    name = "Wall"
    implemented = True
    food_cost = 4
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        list_of_bees_duplicated = list(self.place.bees)
        for bee in list_of_bees_duplicated:
            bee.reduce_armor(self.damage)
        # END Problem 6A
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        QueenAnt.queenCounter+=1
        if self.queenCounter > 1:
            self.imposter = True
        self.allBuffedAnts = []
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            pointerOfPlaceBehindQueen = self.place
            while (pointerOfPlaceBehindQueen.exit == None) == False:
                pointerOfPlaceBehindQueen = pointerOfPlaceBehindQueen.exit
                currentAnt = pointerOfPlaceBehindQueen.ant
                if (currentAnt in self.allBuffedAnts) == False and currentAnt != None:
                    pointerOfPlaceBehindQueen.ant.damage*=2
                    self.allBuffedAnts.append(currentAnt)
                if currentAnt != None and currentAnt.container:
                    if currentAnt.__class__ is TankAnt and (currentAnt in self.allBuffedAnts) == False:
                        currentAnt.damage*=2
                        self.allBuffedAnts.append(currentAnt)
                    if currentAnt.ant:
                        if (currentAnt.ant in self.allBuffedAnts) == False:
                            currentAnt.ant.damage*=2
                            self.allBuffedAnts.append(currentAnt.ant)
...
def apply_effect(effect, bee, duration):
    ...
    nonlocalDuration = duration
    def affectedAction(colony):
        nonlocal nonlocalDuration
        nonlocal originalAction
        if nonlocalDuration > 0:
            nonlocalDuration-=1
            effect(originalAction)(colony)
        else:
            originalAction(colony)
    bee.action = affectedAction
